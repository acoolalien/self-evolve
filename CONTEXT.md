...
| **编码乱码** | Windows 终端编码不匹配导致的中文输出被解码为 `?` 或 `�`（U+FFFD），被噪声过滤直接丢弃 |
...
**决策**：`isNoiseError` 增加编码乱码检测——非空白字符中 `?` 或 `�`（U+FFFD）占比超过 30% 时按噪声丢弃。

**理由**：
- 分析 1464 条历史错误，413 条（28.2%）为 bash 工具产生的编码乱码
- 根因是 Windows 终端编码（CP936/GBK）与 UTF-8 不匹配，liliMozi 已在 OpenHanako 层面修复
- 乱码文本无法被关键词分类器匹配（降级为 unclassified），LLM 也看不出有用模式
- 修复上线后此类错误将自然消亡，噪声过滤充当过渡期兜底

**影响**：`isNoiseError` 需要从 JSON 错误结构中先提取纯文本，再计算乱码占比。

### ADR-16：驳回规则跳过 LLM 分析

**日期**：2026-06-30

**决策**：已标记 `dismissed: true` 的规则再次遇到同类错误时，在 `processFailure` 中直接跳过 LLM 重分析。

**理由**：
- 用户手动删除置顶记忆中的规则 → 不接受该规则 → 不应再消耗 token 精炼它
- 修改前流程：记录错误 → 找到已驳回的规则 → LLM 重分析 → 精炼 → syncPinnedMemory 过滤掉 → 白费一次 LLM 调用
- 修改后：记录错误 → 找到已驳回的规则 → 立即返回

**影响**：`fixRules.get(signature)` 后新增 `if (existingRule.dismissed) return;` 检查。